...
export const useAuth = create((set, get) => ({
  ...
  try {
    ...
    if (remember) {
      localStorage.setItem("auth_state", JSON.stringify({ user, token, refreshToken }));
    } else {
    ...
    if (status === 401) {
      friendly = serverMsg || "Tài khoản hoặc mật khẩu không đúng. Vui lòng thử lại.";
...
register: async (data) => {
  try {
    ...
    if (status === 409 || /already exists|đã tồn tại/i.test(message) || /exists/i.test(code)) {
    ...
    if (status === 400) {
      ...
      const userErr = errors?.Username?.[0] || errors?.username?.[0] || errors?.Account?.[0];
      ...
      if (message) {
        // cố gắng map nhanh
        if (/password/i.test(message)) return new Error("Mật khẩu chưa tuân thủ quy tắc.");
        if (/email.*exist|đã tồn tại/i.test(message)) return new Error("Email đã tồn tại.");
        if (/username|account.*exist|đã tồn tại/i.test(message)) return new Error("Tài khoản đã tồn tại.");
    ...
    // Mặc định
    throw new Error(message || "Không thể tạo tài khoản. Vui lòng thử lại sau.");
  }
},
