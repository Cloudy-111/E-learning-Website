...
//     const res = await http(`${API_BASE}/api/Discussion/ForumQuestion/${id}`, { headers: { accept: "*/*" } });
//     if (!res.ok) return setAnswers([]);
//     const data = await res.json();
//     setAnswers(Array.isArray(data) ? data : (Array.isArray(data?.data) ? data.data : []));
//   };
//   useEffect(() => { fetchAnswers(); /* eslint-disable-next-line */ }, [id]);
...
//     try {
//       setPosting(true);
//       const body = { content }; // nếu BE yêu cầu contentJson => thay thế ở đây
//       const res = await http(`${API_BASE}/api/Discussion/ForumQuestion/${id}`, {
//         method: "POST",
...
//                     {q.authorName || "Người hỏi"} • {new Date(q.createdAt || Date.now()).toLocaleString("vi-VN", { hour12: false })}
//                   </div>
