...
//     try {
//       const res = await http(`${API_BASE}/api/ForumQuestion/${id}`, { method: "DELETE", headers: authHeaders({ accept: "*/*" }) });
//       if (!res.ok) throw new Error(`HTTP ${res.status}`);
//       await fetchMine();
//     } catch (e) { alert("Không xoá mềm được: " + (e.message || "error")); }
//   };

//   const hardDelete = async (id) => {
//     if (!confirm("⚠️ Xoá cứng KHÔNG THỂ khôi phục. Tiếp tục?")) return;
//     try {
//       const res = await http(`${API_BASE}/api/ForumQuestion/${id}/hard`, { method: "DELETE", headers: authHeaders({ accept: "*/*" }) });
//       if (!res.ok) throw new Error(`HTTP ${res.status}`);
//       await fetchMine();
//     } catch (e) { alert("Không xoá cứng được: " + (e.message || "error")); }
//   };

//   const restore = async (id) => {
//     try {
//       const res = await http(`${API_BASE}/api/ForumQuestion/${id}/restore`, { method: "POST", headers: authHeaders({ accept: "*/*" }) });
//       if (!res.ok) throw new Error(`HTTP ${res.status}`);
//       await fetchMine();
//     } catch (e) { alert("Khôi phục thất bại: " + (e.message || "error")); }
//   };
...
//                 {filtered.map((p) => (
//                   <article key={p.id} className="rounded-2xl border bg-white overflow-hidden" style={{ borderColor: BORDER }}>
//                     <div className="p-5">
//                       <div className="text-xs text-slate-500">
//                         {new Date(p.createdAt).toLocaleString("vi-VN", { hour12: false })}
//                         {" • "} {p.isDeleted ? "ĐÃ XOÁ MỀM" : "Đang hiển thị"}
//                       </div>
//                       <h3 className="mt-2 font-semibold text-slate-900 line-clamp-2">{p.title}</h3>
//                       <div className="mt-3 flex flex-wrap gap-2 text-sm">
//                         <Link to={`/forum/${p.id}`} className="rounded-full border px-3 py-1 hover:bg-slate-50">
//                           Xem
//                         </Link>
//                         {!p.isDeleted && (
//                           <button
//                             onClick={() => navigate(`/forum/${p.id}/edit`)}
...
// // ---- NEW: cố gắng xác định memberId/userId từ nhiều nơi (store -> local -> /claims)
// async function getMyMemberIdLazy(user) {
//   // 1) từ store
//   if (user?.memberId) return user.memberId;
//   if (user?.id)       return user.id;
...
//   if (!tk) return null;
//   try {
//     const res = await http(`${API_BASE}/api/Auth/claims`, {
//       headers: authHeaders({ accept: "*/*" }),
//     });
//     if (res.ok) {
//       const claims = await res.json();
//       // BE khác nhau: thử nhiều key phổ biến
//       const candidates = [
//         claims?.nameid,
//         claims?.nameId,
//         claims?.NameId,
...
//         claims?.MemberId,
//         claims?.sub,
//         claims?.StudentId, // phòng khi BE dùng studentId làm memberId
//       ].filter(Boolean);
//       if (candidates.length) {
//         const id = String(candidates[0]);
//         // cache nhẹ để lần sau khỏi gọi
//         try {
//           const old = JSON.parse(localStorage.getItem("auth_user") || "null") || {};
//           localStorage.setItem("auth_user", JSON.stringify({ ...old, id, memberId: id }));
//         } catch {}
//         return id;
//       }
//     }
...
//   // NEW: resolve memberId một lần, rồi mới fetch list
//   useEffect(() => {
//     let alive = true;
//     (async () => {
//       const id = await getMyMemberIdLazy(user);
//       if (alive) {
//         if (!id) {
//           setErr("Không xác định được memberId/userId từ token/claims.");
//           setLoading(false);
//         } else {
//           setMemberId(id);
//         }
...
//     setLoading(true); setErr(null);
//     try {
//       const res = await http(`${API_BASE}/api/ForumQuestion/member/${id}`, {
//         headers: authHeaders({ accept: "*/*" }),
//       });
//       if (!res.ok) throw new Error(`HTTP ${res.status}`);
//       const data = await res.json();
...
//   useEffect(() => { if (memberId) fetchMine(memberId); }, [memberId]);
...
//                       <h3 className="mt-2 font-semibold text-slate-900 line-clamp-2">{p.title}</h3>
//                       <div className="mt-3 flex flex-wrap gap-2 text-sm">
//                         <Link to={`/forum/${p.id}`} className="rounded-full border px-3 py-1 hover:bg-slate-50">Xem</Link>
//                         {!p.isDeleted && (
//                           <button onClick={() => navigate(`/forum/${p.id}/edit`)} className="rounded-full border px-3 py-1 hover:bg-slate-50">Sửa</button>
//                         )}
//                         {!p.isDeleted ? (
//                           <button onClick={() => softDelete(p.id)} className="rounded-full border px-3 py-1 hover:bg-slate-50 text-rose-600">Xoá mềm</button>
//                         ) : (
//                           <>
//                             <button onClick={() => restore(p.id)} className="rounded-full border px-3 py-1 hover:bg-slate-50 text-green-600">Khôi phục</button>
//                             <button onClick={() => hardDelete(p.id)} className="rounded-full border px-3 py-1 hover:bg-slate-50 text-rose-700">Xoá cứng</button>
//                           </>
//                         )}
//                       </div>
//                     </div>
